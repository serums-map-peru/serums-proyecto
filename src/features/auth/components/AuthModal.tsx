...
export function AuthModal({ open, mode, onClose, onChangeMode, onAuthChange }: AuthModalProps) {
  ...
  return (
    ...
    >
      <div
        className={cn(
          "absolute inset-0 bg-slate-900/25 backdrop-blur-[2px] transition-opacity",
          ...
          <div className="p-4">
            {token ? (
              <div className="grid gap-3">
                <div className="text-sm font-semibold text-slate-700">Sesión activa en este navegador.</div>
              ...
              >
                <div className="grid gap-1">
                  <div className="text-xs font-semibold text-slate-600">Código</div>
                  <input
                    value={verifyCode}
                    onChange={(e) => setVerifyCode(e.target.value.replace(/\D/g, "").slice(0, 8))}
                    inputMode="numeric"
                    autoComplete="one-time-code"
                    className="h-10 w-full rounded-xl border border-[var(--border)] bg-white px-3 text-sm font-semibold text-slate-900 outline-none focus:ring-2 focus:ring-blue-100"
                    placeholder="000000"
                    required
                  />
                  <div className="text-xs font-semibold text-slate-500">Enviado a {pendingEmail}</div>
                </div>

                {info ? <div className="rounded-xl bg-blue-50 px-3 py-2 text-sm font-semibold text-blue-800">{info}</div> : null}
                {error ? <div className="rounded-xl bg-red-50 px-3 py-2 text-sm font-semibold text-red-700">{error}</div> : null}
                ...
                <button
                  type="button"
                  className="text-center text-sm font-semibold text-slate-700 underline"
                  disabled={loading}
                  onClick={async () => {
                    setLoading(true);
                    setError(null);
              ...
              >
                ...
                <label className="grid gap-1">
                  <span className="text-xs font-semibold text-slate-600">Contraseña</span>
                  <input
                    value={password}
